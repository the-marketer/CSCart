...
class Valid
{
    ...
    public static function getParam($name = null, $def = null, $toLow = false)
    {
        if (!array_key_exists($name, self::$params)) {
            $p = array_key_exists($name, $_GET) ? $_GET[$name] : $def;

            self::$params[$name] = $toLow === true ? strtolower($p) : $p;
        }

        return self::$params[$name];
    }

    public static function getParamReq($name = null, $def = null, $toLow = false)
    {
        if (!array_key_exists($name, self::$params)) {
            $p = array_key_exists($name, $_REQUEST) ? $_REQUEST[$name] : $def;
                    ...
                    if (self::$error === null) {
                        switch ($do) {
                            ...
                            case 'DateCheckDiscount':
                                if (self::getParam($k) !== null) {
                                    if (!self::validateDate(self::getParam($k), Config::$dateFormat)) {
                                        self::$error = 'Incorrect Date ' .
                                            $k . ' - ' .
                                            self::getParam($k) . ' - ' .
                                            Config::$dateFormat;
                                    }
                                    if (strtotime(self::getParam($k) . ' 23:59:59') < \time()) {
                                        self::$error = 'Incorrect Date ' .
                                            $k . ' - ' .
